...
filepath = "C:/Users/kodai/git/CGLCD_SYSTEM_ESP32C3/tool/imagemap.gif"
...
def cvImageToBinaryString(cvImage):
    ...
    # バイナリの格納先を指定
    ba = np.zeros((np.uint8(HEIGHT/8), WIDTH), dtype=np.uint8)
    ...
    PIL_data.show()
    # 0 or 1 の1行バイナリ列に変換
    #print([0 if x else 1 for x in PIL_data.getdata()])
